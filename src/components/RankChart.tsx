...
const total = items?.reduce((result, item) => result+ item.value, 0) ?? 0   //得到加和值 ??后面是给total初始值  reduce函数可以使用上一次计算的结果作为参数
        ...
        items?.map((item,index) =>
          ...
          <div row-start-2 col-start-2 row-end-3 col-end-4 h-8px self-start
            ...
            <div absolute h-full rounded-4px
              style={{ background: colors[index], width: `${item.value / max * 100}%` }} /> {/**若样式使用到变量那就使用style比较好 */}
          </div>
        </div>)
    )
}
